...
def process_image_and_count(image_path, model_path, classes_to_count, run_dir, iou=0.6, conf=0.2, imgsz=640, augment=False, device='cpu'):
    ...
    class_indices = [classNames.index(cls) for cls in classes_to_count]

    # Process the image with the model
    results = model.track(image, classes=class_indices, persist=True, conf=conf, iou=iou, imgsz=imgsz, augment=augment, stream=False, device=device, half=(device != 'cpu'))
    ...
    for r in results:
        for box in r.boxes:
            if box.id is not None and box.cls[0] in class_indices:
